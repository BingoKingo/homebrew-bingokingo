cask "font-misans-myanmar" do
  version "1.100"
  sha256 "087d8d0b9a11e4897f6060b5fd88964243a9a0193e587a92224ef9a0570063fc"

  url "https://hyperos.mi.com/font-download/MiSans_Myanmar.zip"
  name "MiSans Myanmar VF"
  homepage "https://hyperos.mi.com/font/en/details/myanmar/"

  font "MiSans Myanmar/可变字体/MiSans Myanmar VF.ttf", target: "MiSansMyanmar[wght].ttf"

  caveats do
    <<~EOS
      # Generated by `otfinfo`
      Family:                 MiSans Myanmar VF
      Subfamily:              Regular
      Full name:              MiSans Myanmar VF
      PostScript name:        MiSansMyanmarVF
      Preferred family:       MiSans Myanmar VF
      Preferred subfamily:    Regular
      Version:                Version 1.100
      Unique ID:              1.100;HNYI;MiSansMyanmarVF
      Designer:               Beijing Xiaomi Mobile Software Co.Ltd & Hanyi Fonts
      Copyright:              Copyright © 2023 Beijing Xiaomi Mobile Software Co.,Ltd. All Right Reserved.
      Vendor ID:              HNYI
      Permissions:            Editable
      Axis 0:                 wght
      Axis 0 name:            Weight
      Axis 0 range:           150 700
      Axis 0 default:         330
      Instance 0 name:        Thin
      Instance 0 position:    150
      Instance 1 name:        ExtraLight
      Instance 1 position:    200
      Instance 2 name:        Light
      Instance 2 position:    250
      Instance 3 name:        Normal
      Instance 3 position:    305
      Instance 4 name:        Regular
      Instance 4 position:    330
      Instance 5 name:        Medium
      Instance 5 position:    380
      Instance 6 name:        Demibold
      Instance 6 position:    450
      Instance 7 name:        Semibold
      Instance 7 position:    520
      Instance 8 name:        Bold
      Instance 8 position:    630
      Instance 9 name:        Heavy
      Instance 9 position:    700
    EOS
  end
end
